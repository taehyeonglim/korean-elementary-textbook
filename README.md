...
2022 개정 교육과정에 연결된 무료 초등 수학 학습지를 이미지로 탐색하고 PDF로 내려받는 정적 사이트입니다. 현재는 콘텐츠 투입 전의 안전한 제작·배포 골격이며, 승인된 학습지 데이터만 공개 카탈로그에 추가됩니다.
...
콘텐츠는 `content/workbooks/*.json`에 추가하며, 비공개 MCP 원본 스냅샷은 `content/private/`에만 보관합니다. 공개 가능 상태인 학습지만 `published: true`로 설정할 수 있습니다.
